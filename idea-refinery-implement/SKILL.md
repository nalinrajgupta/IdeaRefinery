...
3. Run enabled unconditional `before_implement` hooks exactly once per implementation run, before baseline tests or protected snapshots. Do not evaluate conditional hooks locally. Record hook output and changed paths; any later hook-driven mutation invalidates affected evidence.
