...
The deterministic validation sidecar is provider- and credential-independent. It validates replayable checklist transitions and cannot make approvals, invoke providers, or run a scheduler. It requires a non-empty completion checklist and a persisted terminal verdict drawn from the supported labels, and rejects anything else instead of normalizing it into success. It completes an item only from recorded transition evidence—an executed action result, a scoped grant, or a validator resolution—and records `external-state` when a pending item has none. Evidence already accepted in a pass persists in the returned state even when a later gate stops the drive, and a sidecar-derived missing-evidence blocker is re-evaluated on the next drive so supplied evidence resumes the run. Its evidence informs the foreground controller; it does not replace controller-owned authorization or shared-artifact writes.
